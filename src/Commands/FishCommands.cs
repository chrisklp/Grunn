...
namespace UnderwaterBot.Modules
{
    public class FishCommands : ModuleBase<SocketCommandContext>
    {
     ...
     /*   [Command("fishall")]
        public async Task FishAll()
        {

            Fish allFish = await _fishService.GetAllFish().ConfigureAwait(false);
            await Context.Channel.SendMessageAsync($"{allFish}");
            
            
            List<Fish> fish = await _fishService.GetAllFish().ConfigureAwait(false);
     ...
     */
     /*   [Command("battlefish")]
        public async Task BattleFish(string fishName)
        {
